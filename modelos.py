...
class CargaBase(BaseModel):
    ...
    tipo_autonomia: Literal['WLTP', 'EPA', 'NEDC'] = Field(..., description="Estándar de autonomía (WLTP, EPA, NEDC).")
    autonomia_km_real: float = Field(..., gt=0)
    consumo_kwh_100km: float = Field(..., gt=0)
    tiempo_carga_horas: float = Field(..., gt=0)
    dificultad_carga: Literal['baja', 'media', 'alta'] = Field(..., description="Dificultad percibida de la carga.")
...
class CargaActualizada(BaseModel):
    ...
    tipo_autonomia: Optional[Literal['WLTP', 'EPA', 'NEDC']] = None
    autonomia_km_real: Optional[float] = Field(None, gt=0)
    consumo_kwh_100km: Optional[float] = Field(None, gt=0)
    tiempo_carga_horas: Optional[float] = Field(None, gt=0)
    dificultad_carga: Optional[Literal['baja', 'media', 'alta']] = None
...
class EstacionBase(BaseModel):
    ...
    ubicacion: str = Field(..., min_length=5, max_length=100)  # Dirección o coordenadas
    tipo_conector: Literal['CCS', 'CHAdeMO', 'Tipo 2', 'NACS', 'Otro'] = Field(...)
    potencia_kw: float = Field(..., gt=0)
    num_conectores: int = Field(..., gt=0)
    acceso_publico: bool = Field(...)
    horario_apertura: str = Field(..., min_length=2, max_length=50)  # Ej: 24/7, 8am-8pm
    coste_por_kwh: float = Field(..., ge=0)  # Mayor o igual a cero (gratis)
...
class EstacionActualizada(BaseModel):
    ...
    tipo_conector: Optional[Literal['CCS', 'CHAdeMO', 'Tipo 2', 'NACS', 'Otro']] = None
    potencia_kw: Optional[float] = Field(None, gt=0)
    num_conectores: Optional[int] = Field(None, gt=0)
    acceso_publico: Optional[bool] = None
    horario_apertura: Optional[str] = Field(None, min_length=2, max_length=50)
...
class UsuarioRegistro(BaseModel):
    ...
    # ✅ CORRECCIÓN 1: Añadir max_length=72
    password: str = Field(
        ...,
        min_length=8,
        max_length=72,  # Límite de 72 caracteres para el hash de Bcrypt
        description="Debe contener entre 8 y 72 caracteres y al menos un número."
    )

    # ✅ CORRECCIÓN 2: Asegurar la validación de número aquí también
    @validator('password')
...
class CambioPassword(BaseModel):
    ...
    # ✅ CORRECCIÓN 3: Añadir max_length=72 a las contraseñas
    password_anterior: Optional[str] = Field(None, max_length=72)  # Campo opcional con límite
    ...
    def validate_new_password(cls, v):
        # ✅ CORRECCIÓN 4: Eliminar la comprobación redundante de len(v) < 8.
        # Pydantic ya lo valida con min_length=8 en Field().
        if not any(char.isdigit() for char in v):
