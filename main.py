# main.py - Script Consolidado y Corregido
...
from modelos import UsuarioRegistro, UsuarioLogin, CambioPassword # Nuevos modelos de auth
...
async def auth_middleware(request: Request, call_next):
    ...
    # Añadimos /docs y /openapi.json para la documentación
    public_paths = ["/", "/register", "/login", "/change_password", "/api/register", "/api/login",
                    "/api/change_password", "/static", "/docs", "/openapi.json"]
...
# --------------------- RUTAS DE PÁGINAS HTML (PÚBLICAS - AUTH) ---------------------

@app.get("/", response_class=HTMLResponse, include_in_schema=False, summary="Página de aterrizaje/Login")
async def read_root_public(request: Request):
    """Muestra la página de inicio, que sirve de landing y punto de entrada al login/registro."""
...
# --------------------- RUTAS DE PÁGINAS HTML (PROTEGIDAS) ---------------------

@app.get("/index", response_class=HTMLResponse, summary="Página de Inicio Principal (Protegida)")
async def index_page(request: Request, db: Session = Depends(get_db)):
    """Muestra la página de inicio principal con estadísticas."""
...
@app.post("/api/register", summary="Registrar un nuevo usuario (Desde Formulario)")
async def register_user(
    ...
    try:
        ...
        form_data = await request.form() # CORRECCIÓN: Usar await
...
@app.post("/api/login", summary="Autenticar un usuario y generar una cookie de sesión (Desde Formulario)")
...
@app.post("/api/change_password", summary="Cambiar la contraseña de un usuario (Desde Formulario)")
